...
function SearchForm(e) {
    ...
    function Plots() {
        if (hidePlots === false) {
            ...
            let api_props = '?brand_d=' + state.Brand
            if (state.Model !== '') api_props = api_props + '&model_d=' + state.Model
            if (state.Year[0] !== '') api_props = api_props + '&year_since_d=' + state.Year[0]
            if (state.Year[1] !== '') api_props = api_props + '&year_till_d=' + state.Year[1]
            ...
            return (
                <Grid container spacing={{ xs: 1 }} columns={{ xs: 6, sm: 12 }} align="center">
                    <Grid item xs={12} sm={6}>
                        <MostCommonPlot api_props={api_props} />
                    </Grid>
                    <Grid item xs={12} sm={6}>
